...
class HTTP extends Server {
  ...
  constructor(options) {
    ...
    this.cache = flatCache.load("cache");
  ...
  initRouter() {
    ...
    this.get("/names", async (req, res) => {
      ...
      const limit = valid.uint("limit", 25);
      const offset = valid.uint("offset", 0);
      const state = valid.str("state", null);
      const desc = valid.bool("desc", true);
      ...
      // Can sort Asc and Desc via 'desc' flag
      items.sort(function(a, b) {
        if (b.height !== a.height) {
          return desc ? b.height - a.height : a.height - b.height;
        }
        if (a.name === b.name) {
          return 0;
        }
    ...
    // Gets number of names in a specific state
    this.get("/states", async (req, res) => {
      const valid = Validator.fromRequest(req);
      const cached = valid.bool("cached", true);

      // Load Cache file
      const statesCache = this.cache.getKey("states");
      // Check if cached flag is passed & cache exists & cache is younger than 10min
      if (cached && statesCache && Date.now() - statesCache.cachedOn < 600000) {
        return res.json(200, statesCache);
      }
      ...
      this.cache.setKey("states", {
        data: { open, bidding, reveal, closed },
        cachedOn: Date.now()
      });
    ...
    // Gets number of names in a specific state
    this.get("/leaderboard", async (req, res) => {
      const valid = Validator.fromRequest(req);
      const length = valid.uint("length", 10);
      const cached = valid.bool("cached", true);

      // Load Cache file
      const statesCache = this.cache.getKey(`leaderboard-${length}`);
      // Check if cached flag is passed & cache exists & cache is younger than 1 hour
      if (
        cached &&
        statesCache &&
        Date.now() - statesCache.cachedOn < 36000000
      ) {
        return res.json(200, statesCache);
      }

      //@todo not sure if this is true.
      enforce(length <= 50, "Leaderboard too big");
      enforce(!this.chain.options.spv, "Cannot get names in SPV mode.");
      ...
      while (await iter.next()) {
        ...
        if (info.state === "REVEAL" || info.state === "CLOSED") {
          items.push(info);
          const [history, total] = await this.hnscan.getNameHistory(
            info.name,
            0,
            200
          );

          txHistory.push({ name: info.name, total });
        }
      }

      /// Find highest price domain
      let value = items.sort((a, b) => b.value - a.value).slice(0, length);
      /// Find highest lockup
      let blind = items.sort((a, b) => b.highest - a.highest).slice(0, length);
      /// Find highest bluff
      let unchallenged = items
        .filter(item => item.value === 0)
        .sort((a, b) => b.highest - a.highest)
        .slice(0, length);
      // Find highest Bluff to Win ratio
      let bluff = items
        .filter(item => item.value > 0)
        .sort((a, b) => a.value / a.highest - b.value / b.highest)
        .slice(0, length);
      // Find highest unblinded bid
      let unblinded = items
        .filter(item => item.value === item.highest)
        .sort((a, b) => b.value - a.value)
        .slice(0, length);
      let mostTxs = txHistory
        .sort((a, b) => b.total - a.total)
        .slice(0, length);

      this.cache.setKey(`leaderboard-${length}`, {
        data: {
          value,
          blind,
          unchallenged,
